...
import {
  useDeliveryScheduler,
  type DeliveryPlatform,
  type DeliveryContentType,
} from "@/hooks/useDeliveryScheduler";
...
export function TopicScheduleButton({
  topicId,
  topicTitle,
}: TopicScheduleButtonProps) {
  const { user } = useAuth();
  ...
  const [selectedContentType, setSelectedContentType] =
    useState<DeliveryContentType>("post");
  ...
  const handleSchedule = async () => {
    ...
    selectedPlatforms.forEach((platformId) => {
      scheduleDelivery({
        ...
        autoGenerate: true,
        customPrompt:
          `Generate content about: ${topicTitle}. ${customPrompt}`.trim(),
      });
    });
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedContentType(value as DeliveryContentType)
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
